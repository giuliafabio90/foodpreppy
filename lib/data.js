...
function buildMealEntry(settings, recipes, day, mt, idx, freeSet, assignMap, usedRecipeIds) {
...
export function generatePlan(settings, categories, recipes) {
  ...
  const available = shuffle(slots.filter((s) => !freeSet.has(s.key)));
  let tokens = [];
  ...
  const days = DAYS.map((day) => {
    const meals = [];
    ["colazione", "pranzo", "cena"].forEach((mt) => meals.push(buildMealEntry(settings, recipes, day, mt, 0, freeSet, assignMap, usedRecipeIds)));
    for (let i = 0; i < settings.spuntiniPerDay; i++) meals.push(buildMealEntry(settings, recipes, day, "spuntino", i, freeSet, assignMap, usedRecipeIds));
    return { day, meals };
  });
  const noRecipeCats = {};

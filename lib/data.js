...
export const DEFAULT_SETTINGS = {
  ...
  // Famiglia: persone aggiuntive che condividono lo stesso piano (stessa
  // categoria/ricetta per pasto) ma con un proprio target calorico, che
  // ricalibra solo le quantita (vedi planForCalories piu sotto).
  members: [],
};
...
// Funzione Family: la stessa struttura del piano (quale categoria/ricetta
// e' assegnata a ogni pasto) resta identica, ma le quantita degli
// ingredienti vengono ricalcolate sul target calorico di una persona
// diversa dal titolare dell'account. Non rigenera il piano, non lo
// salva: e' solo una vista derivata al volo.
export function planForCalories(plan, settings, recipes, dailyCalories) {
  if (!plan) return plan;
  const overrideSettings = { ...settings, dailyCalories };
  return {
    ...plan,
    days: plan.days.map((d) => ({
      ...d,
      meals: d.meals.map((m) => {
        if (!m.recipeId || !m.categoryId) return m;
        const recipe = recipes.find((r) => r.id === m.recipeId);
        if (!recipe) return m;
        return { idx: m.idx, ...computeEntry(overrideSettings, m.mealType, m.categoryId, recipe) };
      }),
    })),
  };
}
...
export function generatePlan(settings, categories, recipes, t = (s) => s) {
  ...
  if (freeSet.size !== settings.freeMealsTarget) {
    warnings.push(t("Hai selezionato {n} pasti liberi ma l'obiettivo e {target}.", { n: freeSet.size, target: settings.freeMealsTarget }));
  }
  ...
  if (unallocated.length) {
    ...
    warnings.push(t("{n} pasti richiesti non hanno trovato uno slot compatibile libero: {list}.", { n: unallocated.length, list }));
  }
  if (leftover.length) {
    warnings.push(t("{n} pasti sono rimasti senza categoria assegnata: aumenta le frequenze delle categorie compatibili o riduci i pasti liberi.", { n: leftover.length }));
  }
  ...
  days.forEach((d) => d.meals.forEach((m) => {
    if (m.noRecipe) {
      ...
      noRecipeCats[`${c ? c.name : m.categoryId} / ${t(MEAL_LABELS[m.mealType])}`] = 1;
    }
  }));
  const nrList = Object.keys(noRecipeCats);
  if (nrList.length) warnings.push(t("Nessuna ricetta disponibile per: {list}. Aggiungi ricette in quella categoria/pasto nella sezione Ricette.", { list: nrList.join(", ") }));

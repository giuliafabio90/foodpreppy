...
export default function MacrosView({ settings, setSettings, t }) {
  ...
  return (
    <div className="panel">
      <h3>{t("Range macro per pasto")}</h3>
      <div className="sub">{t("Percentuale di calorie da proteine, carboidrati e grassi accettata per ciascun tipo di pasto. Le percentuali riflettono la composizione della ricetta e non cambiano quando la porzione viene ricalibrata.")}</div>
      {MEAL_TYPES.map((mt) => (
        <div className="range-row" key={mt}>
          <div className="mealname">{t(MEAL_LABELS[mt])}</div>
          {FIELDS.map((f) => {
            const range = settings.macroRanges[mt][f.key];
            return (
              <div key={f.key}>
                <span className="range-label" style={{ color: `var(--${f.color})` }}>{t(f.label)}</span>
